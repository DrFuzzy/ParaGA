...
% inputtestvectors(4)= bin2dec(num2str(seed3{index})); 
...
if isempty(find(seed1{index})) == 1 && current_gen ~= 1 
    LFSR_reg_1 = temp_scale(nParents+1,:);
end
...
if isempty(find(seed2{index})) == 1 && current_gen ~= 1 
...
for i=1:nParents/2+1
    ...
    if i~=1 
        ...
        mut_point{index}(i-1) = cros_mut_points(i,1);
    end
end
...
for i=1:(popSz-elite)
    mut_rng{index}(i,1:genomlngt) = mut_mask{index}(i,1:genomlngt);
    mut_rng{index}(i,genomlngt+1) = bin2dec(num2str(mut_mask{index}(i,genomlngt+1:genomlngt+mut_res)));
end
...
[offsprings{index}] = mutation(offspring{index}, mut_point{index}, mut_rng{index}, mut_method);
...
% outputtestvectors(3)=max_fit{index};
